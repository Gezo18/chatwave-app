...
import { Search, Plus, MessageCircle, LogOut, X, UserCheck, Sun, Moon, User } from 'lucide-react';
...
export default function ChatList({ conversations, activeId, onSelect, onNewChat, onProfile }) {
    ...
    <div className="h-full flex flex-col bg-[var(--bg-light)] border-r border-[var(--border)]">
      <div className="p-4 border-b border-[var(--border)]">
        ...
        <div className="flex items-center justify-between mb-4">
          <button onClick={onProfile} className="flex items-center gap-3 hover:opacity-80 transition-opacity">
        ...
        {tab === 'chats' ? (
          filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-[var(--text-muted)] p-4">
              ...
              <button onClick={onNewChat} className="mt-2 text-sm text-[var(--primary)] hover:underline">Start a new chat</button>
            </div>
          ) : (
            <div className="py-1">
              {filtered.map((conv) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 hover:bg-[var(--bg)] transition-colors ${activeId === conv.id ? 'bg-[var(--bg)]' : ''}`}
                  >
                    <Avatar name={conv.name} color={conv.avatar} online={conv.type === 'direct' ? isOnline : undefined} />
                    <div className="flex-1 min-w-0 text-left">
                      ...
                      <div className="flex items-center justify-between mt-0.5">
                        ...
                        <div className="flex items-center gap-1">
                          {conv.muted && <span className="text-xs text-[var(--text-muted)]">🔇</span>}
                          {conv.unread > 0 && (
                            <span className="ml-1 px-2 py-0.5 bg-[var(--primary)] text-white text-xs rounded-full font-medium">
                              {conv.unread}
                            </span>
                          )}
                        </div>
              ...
              {filteredContacts.map((contact) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    existing ? onSelect(existing) : onNewChat();
                  }}
                  className="w-full flex items-center gap-3 px-4 py-3 hover:bg-[var(--bg)] transition-colors"
                >
                  <Avatar name={contact.username} color={contact.avatar} online={onlineUsers.has(contact.id)} />
                  <div className="flex-1 min-w-0 text-left">
